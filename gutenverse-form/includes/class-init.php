				...
				if ( version_compare( $form['Version'], $required_version, '<' ) && is_plugin_active( $plugin ) ) {
					add_action(
						...
						function () use ( $instance, $plugin_name, $required_version ) {
							...
							// translators: %s is plugin name.
							$notice_description = "We notice that you haven't update %s plugin to version {$required_version} or above but, currently using Gutenverse Form version 3.0.0 or above.";
