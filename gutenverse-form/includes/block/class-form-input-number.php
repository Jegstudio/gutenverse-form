...
/**
 * Class Form_Input_Number Block
 *
 * @package gutenverse-form\block
 */
class Form_Input_Number extends Block_Abstract {
	/**
	 * Render content
	 *
	 * @return string
	...
	public function render_content() {
		...
		$input_min          = isset( $this->attributes['inputMin'] ) ? $this->attributes['inputMin'] : '';
		$input_max          = isset( $this->attributes['inputMax'] ) ? $this->attributes['inputMax'] : '';
		$input_step         = isset( $this->attributes['inputStep'] ) ? $this->attributes['inputStep'] : '';
		...
		$input_html = '<input
            ...
            type="number"
            min="' . esc_attr( $input_min ) . '"
            max="' . esc_attr( $input_max ) . '"
            step="' . esc_attr( $input_step ) . '"' . $additional_props . '/>';

		if ( $use_icon ) {
			$html  = '<div class="input-icon-wrapper input-number">';
	...
	protected function icon_content() {
		...
		$icon            = isset( $this->attributes['icon'] ) ? $this->attributes['icon'] : 'gtn gtn-sort-numeric-up-solid';
		...
		switch ( $icon_type ) {
			case 'icon':
			case 'svg':
				$html .= '<div class="form-input-number-icon type-icon">';
			...
			case 'image':
				if ( ! empty( $image ) ) {
					...
					$html .= '<div class="form-input-number-icon type-image">';
					$html .= '<div class="icon style-' . esc_attr( $icon_style_mode ) . '">';
	...
	public function render_frontend() {
		...
		$input_label        = isset( $this->attributes['inputLabel'] ) ? $this->attributes['inputLabel'] : 'Number';
		...
		$class_name = 'guten-element guten-form-input-number guten-form-input guten-input-position-' . $position . ' ' . $element_id . $display_classes . $animation_class . $custom_classes;
