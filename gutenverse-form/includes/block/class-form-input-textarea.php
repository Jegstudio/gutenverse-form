<?php
/**
 * Form Input Textarea Block class
 *
 * @author Jegstudio
 * @since 1.0.0
...
/**
 * Class Form_Input_Textarea Block
 *
 * @package gutenverse-form\block
 */
class Form_Input_Textarea extends Block_Abstract {
	/**
	 * Render content
	 *
	 * @return string
	...
	public function render_content() {
		$input_placeholder  = isset( $this->attributes['inputPlaceholder'] ) ? $this->attributes['inputPlaceholder'] : 'Textarea Placeholder';
		$input_name         = isset( $this->attributes['inputName'] ) ? $this->attributes['inputName'] : 'input-textarea';
		...
		$input_html = '<textarea
            ...
            class="gutenverse-input gutenverse-input-text"' . $additional_props . '></textarea>';

		if ( $use_icon ) {
			$html  = '<div class="input-icon-wrapper input-textarea">';
	...
	protected function icon_content() {
		...
		$icon            = isset( $this->attributes['icon'] ) ? $this->attributes['icon'] : 'gtn gtn-text-size-light';
		...
		switch ( $icon_type ) {
			case 'icon':
			case 'svg':
				$html .= '<div class="form-input-textarea-icon type-icon">';
			...
			case 'image':
				if ( ! empty( $image ) ) {
					...
					$html .= '<div class="form-input-textarea-icon type-image">';
					$html .= '<div class="icon style-' . esc_attr( $icon_style_mode ) . '">';
	...
	public function render_frontend() {
		...
		$input_label        = isset( $this->attributes['inputLabel'] ) ? $this->attributes['inputLabel'] : 'Textarea';
		...
		$class_name = 'guten-element guten-form-input-textarea guten-form-input guten-input-position-' . $position . ' ' . $element_id . $display_classes . $animation_class . $custom_classes;
